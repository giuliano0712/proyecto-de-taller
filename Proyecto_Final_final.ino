...
int servoPin = 9; //Pin de señal al servo
...
int direccionActual = direccionCentro; // Para inicializar el auto con el servo centrado
int movimientoActual = 0;  // 0 = detenido, 1 = adelante, -1 = atrás
void setup() {
  // Configuración del Bluetooth y del puerto serie
  Serial.begin(9600);
  bluetooth.begin(9600); //Inicializo el modulo bluetooth
...
void loop() {
  ...
  if (bluetooth.available()) {
    ...
    Serial.println(comando); //Imprimo el comando recibido para conocer las señales de entrada
